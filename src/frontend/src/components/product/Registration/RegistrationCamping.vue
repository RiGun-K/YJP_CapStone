		...
		<div id="content">
			...
			>
				<div>
					...
					<div class="mt-3">
						선택유형 : <strong>{{ infoterid.text }}</strong>
					</div>
					...
					<select v-model="bigPick" @change="bigCheck(bigPick)">
						...
						<option v-for="big in bigRound" :value="big.areaId">
							{{ big.areaName }}
						</option>
					</select>
					<br />
				...
				<div>
					...
					<span class="box int_email">
						<input
							type="text"
							v-model="campingDetailState"
							id="email"
							class="int"
							maxlength="100"
							placeholder="객실 수를 입력하세요"
						/>
					</span>
				</div>
	...
	methods: {
		bigCheck(index) {
			if (index == '0') {
				...
				axios
					.get('/api/campingSmallRound/' + index)
					.then((res) => {
						console.log(res.data);
		...
		BuySubmit: function () {
			...
			axios
				.post('/api/Camping_Signup', formData, {
				...
				.then((res) => {
					...
					if (confirm('캠핑장을 등록하시겠습니까?')) {
						axios
							.post('/api/Camping_Signup_Files/', formDatas, {
								headers: {
