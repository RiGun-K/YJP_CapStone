<template>
  ...
  <div>
    ...
    <table class="table table-striped">
      ...
      <tbody>
      <tr v-for="RentalList in list"
          :key="RentalList.id"
          :item="RentalList" @click="showInfo(RentalList)" style="cursor:pointer;">
        <th scope="row">{{ RentalList.kindid.kindname }}</th>
        <td>{{ RentalList.rentalName }}</td>
        <td>{{ RentalList.rentalStock }}</td>
        <td>{{ RentalList.savedTime }}</td>
      </tr>
...
export default {
  ...
  methods: {
    goMyData() {
      ...
      axios.get('http://localhost:9002/api/Rental_List/'+this.user)
          .then((res) => {
