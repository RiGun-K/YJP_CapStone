      ...
      <form v-on:submit.prevent="formSubmit" method="post" enctype="multipart/form-data">
        ...
        <div>
          ...
          <span class="box int_id">
                        <input type="text" v-model="myContent.rentalName" id="id" placeholder="상품명을 입력하세요" class="int" maxlength="20">
                    </span>
          <span class="error_next_box"></span>
        </div>
        ...
        <div>
          ...
          <span class="box int_id">
          <input type="text" v-model="myContent.rentalStock" id="id" placeholder="상품 수량을 입력하세요" class="int" maxlength="100">
                    </span>
          <span class="error_next_box"></span>
        ...
        <div>
          ...
          <span class="box int_email">
            <input type="text" v-model="myContent.rentalPrice" id="email" class="int" maxlength="100" placeholder="상품 가격을 입력하세요">
          </span>
        </div>
        ...
        <div>
          ...
          <span class="box int_email">
            <input type="text" v-model="myContent.rentalEx" id="email" class="int" maxlength="100" placeholder="상품 설명을 입력하세요">
          </span>
        </div>
...
export default {
  ...
  methods: {
    myDataList() {
      ...
      axios.get('http://localhost:9002/api/Rental_detail/' + this.id)
          .then(res => {
    ...
    updated() {
      ...
      formData.append('rentalId', this.id);
      formData.append('kindid', this.kindid);
      formData.append('rentalName', this.myContent.rentalName);
      formData.append('rentalPrice', this.myContent.rentalPrice);
      formData.append('rentalStock', this.myContent.rentalStock);
      formData.append('rentalEx', this.myContent.rentalEx);
      formData.append('file', this.file);
      formData.append('mid', this.mid);



      console.log(this.id, this.kindid, this.myContent.rentalName, this.myContent.rentalStock, this.myContent.rentalPrice, this.myContent.rentalEx, this.file, this.mid);
      const baseURI = 'http://localhost:9002';

      if (confirm("상품을 수정하시겠습니까?")) {
        axios.put(`${baseURI}/api/Rental_Update`, formData, {headers: {'Content-Type': 'multipart/form-data'}})
            .then(res => {
              console.log("성공" + res);
              alert("상품이 수정되었습니다.");
              this.$router.push({
    ...
    deleted() {
      if(confirm("삭제하시겠습니까?")) {
        axios.delete('http://localhost:9002/api/Rental_Delete/' + this.id)
            .then(res => {
